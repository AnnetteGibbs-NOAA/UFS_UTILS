...
! zm - Not used yet. Xu li said set to '0'.
  ...
  if (localpet == 0) then
! Do not set zm to 0 for warm restart files - AMG
    if (trim(input_type) == "restart") then
      print*,"- Do not set zm to 0 for warm restart files"
      call read_fv3_grid_data_netcdf('zm', tile, i_input, j_input, &
                                     lsoil_input, sfcdata=data_one_tile)
!     print*,'zm max/min ',maxval(data_one_tile),minval(data_one_tile)
    else
      data_one_tile = 0.0
    endif
  endif
